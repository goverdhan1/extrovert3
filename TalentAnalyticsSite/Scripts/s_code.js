...
2/18/15 CW: Added omniPageView() and omniClearVars().
2/25/15 CW: Updated the above function names to analyticsPageTrack() and analyticsClearVars().
*/
...
function s_doPlugins(s) {
    ...
    if (dllink) {
        ...
        s.linkTrackVars = s.apl(s.linkTrackVars, 'prop31,prop33,prop32,eVar32,eVar31,prop75,eVar75,prop14,eVar14,events', ',', 2);
...
// Page view tracking functionality wrapped in a function.  Pass in standard implementation information: page name, site section, user id.
function analyticsPageTrack(pageName, channel, userID) {
    var s = s_gi(s_account);
    analyticsClearVars();
    s.pageName = pageName;
    s.channel = channel;
    s.prop14 = s.eVar14 = userID;
    s.t();
}

// Utility function to clear out most variables.
function analyticsClearVars() {
    var s = s_gi(s_account);
    for (var i = 1; i <= 75; i++) {
        s['prop' + i] = '';
        s['eVar' + i] = '';
    ...
    svarArr = ['pageName', 'channel', 'events', 'campaign', 'server'];
